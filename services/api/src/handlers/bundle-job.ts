...
export async function handleBundleJob(event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> {
  ...
  try {
    ...
    const objects = listResp.Contents?.filter((obj) => obj.Key && !obj.Key.endsWith('/'));
